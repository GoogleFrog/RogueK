...
local function GetClosestCell(point, nearCells, satFunc)
	if not nearCells[1] then
		return false
	end
	local closeIndex = false
	local closeDistSq = false
	for i = 1, #nearCells do
		if (not satFunc) or satFunc(nearCells[i]) then
			local thisDistSq = DistSq(point, nearCells[i].site)
			if (not closeIndex) or thisDistSq < closeDistSq then
				closeIndex = i
...
local function RepelFromNearestPoint(points, index, forceMult, distBase)
	local point = points[index]
	local nearID, nearDist = GetClosestPoint(point, points, false, index)
	local vector = Subtract(points[nearID], point)
	local force = Mult(-1 * forceMult / (nearDist * nearDist + distBase), vector)
	points[index] = PutPointInMap(Add(point, force), 20)
end
...
local function DoPointSplit(points, radius, ignoreSplit)
	...
	for i = 1, #points do
		...
		if ((not SYMMETRY) or i < point.mirror) and not (ignoreSplit and ignoreSplit[i]) then
...
local function MakeRandomPoints(params)
	...
	if params.repelIterations and not SYMMETRY then
		for i = 1, params.repelIterations do
			for j = 1, #points do
				RepelFromNearestPoint(points, j, params.repelForce, params.repelBase)
				--Spring.MarkerAddPoint(points[j][1], 0, points[j][2], i)
			end
		end
	end
	if splitRadius then
		for i = 1, params.splitIterations or 1 do
...
local function GetWave(translational, params)
	...
	local xOffset    = (SYMMETRY and 0) or (-(params.horOffset or 0) + (2 * (params.horOffset or 0))*random())
	local zOffset    = (SYMMETRY and 0) or (-(params.horOffset or 0) + (2 * - (params.horOffset or 0))*random())
...
local function GetNonSymmetryWaveFunction()
	local multParams = {
		scaleMin = 0.65,
		scaleMax = 0.8,
		periodMin = 2000,
...
local function GenerateEdgePassability(params, cells, edgesSorted, waveFunc, waveHeightMult, tierMin, tierMax, tierFunc, minLandTier)
	...
	for i = #edgesSorted, 1, -1 do
		local thisEdge = edgesSorted[i]
		if (not SYMMETRY) or thisEdge.firstMirror then
	...
	for i = 1, #cells do
		...
		if thisCell.firstMirror and (thisCell.metalSpots or 0) == 0 then
			...
			if SYMMETRY then
				...
				thisCell.startDistFactor = (thisCell.straightDist - minDistSum)/(maxDistSum - minDistSum)
				thisCell.closeDistFactor = thisCell.straightDist/maxCellDist
			end
